 ...
 * diagonal terms of the matrix. \\(\sum_{i}^{Nr} A_{(i,i)}\\).
 *
 * @tparam _Tp Value type of the container.
 * @tparam _Nr Number of rows in the container.
 ...
 * suited for matricies with on order less than \\(2^{10}\\), matricies larger
 ...
 * complexity of \\(O(n^2)\\). If an efficient algorithm is needed for larger
